// Left Navigation Panel - Shared across all pages
// This file should be included in all pages to render the consistent navigation

function renderLeftNaviPanel(currentPage) {
    const navItems = [
        {
            id: 'chat',
            label: 'Chat',
        ...
        },
        {
            ...
            label: 'Config',
            href: '/config.html',
            title: 'Configure LLM models and settings',
    ...
    const navHtml = `
        <aside class="sidebar">
            ...
            <nav class="nav-links">
                ${navItems.map(item => `
                    <a href="${item.href}" class="nav-link ${item.id === currentPage ? 'active' : ''}" ${item.title ? `title="${item.title}"` : ''}>
                        ${item.icon}
                        ${item.label}
                    </a>
                `).join('')}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (container) {
        ...
        container.innerHTML = renderLeftNaviPanel(currentPage);
    }
});
